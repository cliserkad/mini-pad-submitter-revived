 ...
 * [ST.java]
 *
 * Summary: Miscellaneous static methods for dealing with Strings in JDK 1.8+.
 *
 ...
 *  3.1 2015-11-02 add last and tail methods
 */
...
public class ST
     ...
     * Reader's digest condensed books were still readable. We do not squish.
     *
     ...
     * @see #squish(String)
     * @see com.mindprod.common18.ST#condense(String)
     */
     ...
     * @param count of chars to include
     *
     * @return last chars of the String.
     */
    public static String last( String s, int count )
        {
        final int start = s.length() - count;
        if ( start < 0 )
            {
            return s;
            }
    ...
    /**
     * insert spaces between the letters L I K E  T H I S
     *
    ...
    /**
     * Strip out any commas since they confuse parse.
     *
     * @param amount string of digits decimal point and optional commas.
     *
     * @return amount with any commas removed.
     */
    public static String stripCommas( String amount )
        {
        return stripNaughtyCharacters( amount, "," );
        }
     ...
     * @return last chars of the String.
     */
    public static String tail( String s, int start )
        {
        if ( start >= s.length() )
            {
            return s;
    ...
    /**
     * Pluralise a string like 0 ducks, 1 duck, 2 ducks.
     *
     * @param count how many items there are.
     * @param what  that the items are.
     *
     * @return "2 ducks"
     */
    public static String things( int count, String what )
        {
    ...
    /**
     * Convert an  integer as STring to a String, with left zeroes.
     *
     ...
     * @see #leftPad
     */
    public static String toLZ( String s, int len )
        {
        // Since String is final, we could not add this method there.
        if ( s.length() > len )
            {/* return rightmost len chars */
    ...
    /**
     * prune junk characters from bead, tail and middle of a string
     *
     * @param text      text to clean
     * @param junkChars chars to consider as junn
     *
     * @return string with any of those junk characters removed.
     */
    public static String trimJunk( String text, String junkChars )
        {
        final StringBuffer sb = new StringBuffer( text.length() ); //do not convert to FastCat
        for ( int i = 0; i < text.length(); i++ )
            {
            final char c = text.charAt( i );
            if ( junkChars.indexOf( c ) < 0 )
                {
                sb.append( c );
                }
            }
    ...
    /**
     * trim trailing characters there are on a string matching a given character, could be more than one.
     *
     * @param text text with possible trailing characters, possibly empty, but not null.
     * @param c    the trailing character of interest, usually ' ' or '\n'
     *
