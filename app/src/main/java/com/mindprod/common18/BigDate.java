 ...
 * [BigDate.java]
 *
 * Summary: Manipulate pure dates, without time or time zone.
 *
 ...
 *  4.7 2005-07-15 isValid( yyyy-mm-dd), new package, com.mindprod.common18,
 *                 getCopyright
 ...
 *  5.3 2008-12-15 toString now handles BC and non-4-digit years. toYYYYString,
 *                 Constructor handles BC and non-4-digit years.
 *  5.4 2009-04-29 add parseYYYYmmdd to parse incomplete date strings.
 *  5.5 2009-05-05 add isAnniversary
 *  5.6 2009-12-09 parsing of String dates is now more relaxed. Now ignores spaces,
 *                 commas. Accepts single digit months and days.
 *  5.7 2011-02-09 add toOrdinal ( yyyy_mm_dd ), throw NumberFormatException instead of IllegalArgumentException.
 *  5.8 2014-04-29 add isCelebrating. Give all static final constants pure upper case names.
 *  5.9 2014-08-09 make constructors null-safe. Null generates a NULL_ORDINARY BigDate.
 */
 ...
 * datestamp form). BigDate objects are not designed for <strong>storing</strong> dates in a database,
 * just for conversion. Long
 ...
 * <p/>
 * 1900  is not a leap year (mod 100): <strong>yes</strong>.
 * <p/>
 * 2000 is a leap year (mod 400): <strong>yes</strong>.
 * <p/>
 * The 10 missing days in 1582 October.: <strong>yes</strong> (Pope Gregory's correction) 1582-10-05 to 1582-10-14 never
 * happened.
 * <p/>
 * Britain and its territories (including the USA and Canada) adopted the Gregorian correction in 1752:
 * <strong>Yes</strong>.  By
 * then, 11 days had to be dropped. 1752-09-03 to 1752-09-13 never happened. However, you can modify
 * constants in BigDate to use the British calendar. Such a change only affects dates prior to 1753 since BigDate
 * calendar is based on 1970-01-01. toOrdinal with the Gregorian and British scheme will give the same number for
 * recent dates. <strong>You must recompile BigDate with the <strong>IS_BRITISH</strong> boolean changed to true
 * .</strong>. It was used by
 * Britain and its colonies which later became the USA and Canada.  However Nova Scotia used Pope Gregory's calendar.
 * see http://mindprod.com/jgloss/missingdays.html PLEASE CONFIGURE IS_BRITISH AND RECOMPILE BigDate IF YOU WISH TO USE
 * THE BRITISH CALENDAR.
 * <p/>
 * missing year 0 between 1 BC and 1 AD. <strong>yes</strong>.
 * <p/>
 * in Roman times leap years occurred at irregular intervals, Considered inauspicious, they were avoided during war.
 * <strong>no</strong>. Instead we presume leap years every 4 years even back to 999,999BC.
 * <p/>
 * leap seconds: <strong>no</strong>
 * <p/>
 ...
 * @version 5.9 2014-08-09 make constructors null-safe. Null generates a NULL_ORDINARY BigDate.
...
public final class BigDate implements Cloneable, Serializable, Comparable
    {
    /**
     * PLEASE CONFIGURE IS_BRITISH BEFORE COMPILING. Mysterious missing days in the calendar. Pope Gregory: 1582-10-04
     ...
     * true.
     */
    public static final boolean IS_BRITISH = true;
    ...
    /**
     * Constant: earliest ordinal that BigDate handles.
     * It corresponds to 999,999 Jan 01 BC.
     */
     ...
     * e.g.
     * 1954-09-31 -> 1954-10-01.
     * 1954-10- minus 1 -> 1954-09-30
    ...
    /**
     * used to identify this version of serialised BigDate objects (leave camel case)
     */
     ...
     */
    private static final int CG_FIRST_MM;

    /**
     * Constant: day of the first day of the month of the Gregorian Calendar Pope Gregory: 1582-10-15, British: 1752
     * Sep 14
     */
    private static final int GC_FIRST_DD;

    /**
     * Constant: year of the first date (1752-10-15) of the Gregorian Calendar = 1582. Just after the missing days.
     * Different parts of the world made the transition at different times. Pope Gregory: 1582-10-04,
     * British: 1752-09-02.
     */
    private static final int GC_FIRST_YYYY;
     ...
     */
    private static final int LEAP_100_RULE_YYYY;
     ...
     */
    private static final int LEAP_400_RULE_YYYY;

    /**
     * Constant array: how many days in the year prior to the first of the given month in a leap year. Indexed by
     * Jan=0.
     */
    private static final int[] LEAP_DAYS_IN_YEAR_PRIOR_TO_MONTH_TABLE = {
            0, 31, 60, /* J F M */
            91, 121, 152, /* A M J */
            182, 213, 244, /* J A S */
            274, 305, 335 /* O N D */ };
     ...
     */
    private static final int[] USUAL_DAYS_PER_MONTH_TABLE = {
            31, 28, 31, /* J F M */
            30, 31, 30, /* A M J */
            31, 31, 30, /* J A S */
            31, 30, 31 /* O N D */ };
     ...
     */
    private static final int[] USUAL_DDD_TO_MM_TABLE;

    /**
     * days of the week 3 letter abbreviations in English. Index Sunday = 0.
     */
    private static final String[] DAY_ABBRS =
     ...
     */
    private static final String[] MONTH_ABBRS = {
            "???",
            "Jan",
            "Feb",
            "Mar",
    ...
    static
        {
        /*
         Initialisation order is tricky so we collect all the dependencies here.
         This way no matter how fields are ordered, initialisation still works.
        */
        // set IS_BRITISH at the top
        NORMALIZE = NORMALISE;
        OJC_LAST_YYYY = IS_BRITISH ? 1752 : 1582;
        OJC_LAST_MM = IS_BRITISH ? 9 : 10;
        OJC_LAST_DD = IS_BRITISH ? 2 : 4;
        GC_FIRST_YYYY = IS_BRITISH ? 1752 : 1582;
        CG_FIRST_MM = IS_BRITISH ? 9 : 10;
        GC_FIRST_DD = IS_BRITISH ? 14 : 15;
        LEAP_100_RULE_YYYY = ( ( GC_FIRST_YYYY + 99 ) / 100 ) * 100;
        LEAP_400_RULE_YYYY = ( ( GC_FIRST_YYYY + 399 ) / 400 ) * 400;
        MISSING_DAYS = GC_FIRST_DD - OJC_LAST_DD - 1;
        AD_EPOCH_ADJUSTMENT = -719529;
        BC_EPOCH_ADJUSTMENT = AD_EPOCH_ADJUSTMENT + 365;
        MIN_ORDINAL = BigDate.toOrdinal( MIN_YEAR, 1, 1 );
        ORDINAL_0001_01_01BC = BigDate.toOrdinal( -1, 1, 1 );
        ORDINAL_0001_01_01AD = BigDate.toOrdinal( 1, 1, 1 );
        ORDINAL_0004_01_01AD = BigDate.toOrdinal( 4, 1, 1 );
        ORDINAL_GC_FIRST =
                BigDate.toOrdinal( GC_FIRST_YYYY, CG_FIRST_MM, GC_FIRST_DD );
        ORDINAL_GC_FIRST_DEC_31 = BigDate.toOrdinal( GC_FIRST_YYYY, 12, 31 );
        ORDINAL_LEAP_RULE_100_START = BigDate.toOrdinal( LEAP_100_RULE_YYYY, 1, 1 );
        ORDINAL_LEAP_RULE_400_START = BigDate.toOrdinal( LEAP_400_RULE_YYYY, 1, 1 );
        MAX_ORDINAL = BigDate.toOrdinal( MAX_YEAR, 12, 31 );
        }

    static
        {
        // <clinit> static initialisation code for USUAL_DDD_TO_MM_TABLE
        USUAL_DDD_TO_MM_TABLE = new int[ 365 ];
        int dddi = 0;
        for ( int mmi = 1; mmi <= 12; mmi++ )
            ...
            for ( int ddi = 0; ddi < last; ddi++ )
                {
                USUAL_DDD_TO_MM_TABLE[ dddi++ ] = mmi;
                } // end for dd
            } // end for mmm
        } // end static init
    ...
    static
        {
        // <clinit> static initialisation code for LEAP_DDD_TO_MM_TABLE
        LEAP_DDD_TO_MM_TABLE = new int[ 366 ];
        int dddi = 0;
        for ( int mmi = 1; mmi <= 12; mmi++ )
            {
     ...
     * encode year 1 BC as 0, and convert on output. That would simplify calculation over the 1AD -1BC barrier.
     *
     * @noinspection WeakerAccess
     */
     ...
     * Default constructor uses CHECK.  Initial value suggest initial value is not trusted.
     */
    private int how = BYPASSCHECK;
    ...
    public BigDate( String yyyy_mm_dd )
        ...
        // will throw NumberFormatException  for an invalid date.
     ...
     *
     * @throws NumberFormatException for invalid yyyy mm dd
     */
    public BigDate(
            int yyyy,
            int mm,
    ...
    protected static int daysInYearPriorToMonth( int mm, boolean leap )
        {
        return leap
               ? LEAP_DAYS_IN_YEAR_PRIOR_TO_MONTH_TABLE[ mm - 1 ]
               : USUAL_DAYS_IN_YEAR_PRIOR_TO_MONTH_TABLE[ mm - 1 ];
        } // /method
    ...
    protected static int dddToMM( int ddd, boolean leap )
        {
        return leap
               ? LEAP_DDD_TO_MM_TABLE[ ddd - 1 ]
               : USUAL_DDD_TO_MM_TABLE[ ddd - 1 ];
        } // /method
    ...
    protected static int jan01OfYear( int yyyy )
        ...
        if ( yyyy < 0 )
            ...
            return ( yyyy * 365 ) - leapAdjustment + BC_EPOCH_ADJUSTMENT;
            }
        // years 4 8 12 were leap years
        // adjustment 1->0 2->0, 3->0, 4->0, 5->1, 6->1, 7->1, 8->1, 9->2
        leapAdjustment = ( yyyy - 1 ) / 4;
        int missingDayAdjust = ( yyyy > GC_FIRST_YYYY ) ? MISSING_DAYS : 0;
        // mod 100 and mod 400 rules started in 1600 for Gregorian,
        // but 1800/2000 in the British scheme
        if ( yyyy > LEAP_100_RULE_YYYY )
            {
            leapAdjustment -= ( yyyy - LEAP_100_RULE_YYYY + 99 ) / 100;
            }
        if ( yyyy > LEAP_400_RULE_YYYY )
            {
            leapAdjustment += ( yyyy - LEAP_400_RULE_YYYY + 399 ) / 400;
            }
        return yyyy * 365 + leapAdjustment - missingDayAdjust
               + AD_EPOCH_ADJUSTMENT;
        } // /method

    /**
     * parse a string into yyyy mm dd, where date may have form:
     * yyyy-mm-dd
     * y-mm-ddBC
     * 2009-12-31
     * 1234-5-31AD
     ...
     * 2000_12_31
     * It will not accept a date missing the year, month or day field.  Spaces and commas are ignored.
     * It does does not check leap year validity, max days in month.
     *
     ...
     * e.g. 1954-09-31 -> 1954-10-01.
     * 1954-10-(minus 1) -> 1954-09-30
     * 1954-13-01 -> 1955-01-01. This lets you do year, month or day
     * arithmetic. normalise does not recompute the ordinal.
     */
    protected final void normalise()
        ...
        if ( isValid( yyyy, mm, 1 ) )
            {
            final int oldDD = dd;  // poss 0 or negative
            dd = 1;
            calcOrdinal();
            ordinal += oldDD - 1;
            toGregorian();
        ...
        throw new NumberFormatException( "date cannot be normalised: "
                                         + yyyy
                                         + "/"
                                         + mm
                                         + "/"
    ...
    protected final void toGregorian()
        ...
        if ( ordinal > MAX_ORDINAL )
            {
            throw new NumberFormatException( "invalid ordinal date: "
                                             + ordinal );
            }
        else if ( ordinal >= ORDINAL_GC_FIRST )
            {
            yyyy =
                    LEAP_400_RULE_YYYY + flooredMulDiv( ordinal
                                                        - ORDINAL_LEAP_RULE_400_START,
                            10000,
                            3652425
        ...
        else if ( ordinal >= ORDINAL_0001_01_01AD )
            {
            // ORDINAL_0001_01_01AD to Oct_04_1582
            // year 4 was first AD leap year.
            yyyy = 4 + flooredMulDiv( ordinal - ORDINAL_0004_01_01AD, 100, 36525 );
            /* 365 + 0.25 */
            }
            ...
            yyyy = -1 + flooredMulDiv( ordinal - ORDINAL_0001_01_01BC, 100, 36525 );
            /* 365 + 0.25 */
            }
        else
            {
            throw new NumberFormatException( "invalid ordinal date: " + ordinal );
            }
        int aim = ordinal + 1;
        // Oct_15_1582 <= ordinal && ordinal <= Dec_31_1582
        if ( ORDINAL_GC_FIRST <= ordinal && ordinal <= ORDINAL_GC_FIRST_DEC_31 )
            {
            aim += MISSING_DAYS;
            }
        // ddd should be 1..366
    ...
    /**
     * s a BigDate object initialised to today's UTC (Greenwich GMT) date, in other words the date and time in
     ...
     *
     * @return BigDate object initialised to today, in Greenwich. You may modify it further.
     * @see #localToday
     * @see #today
     */
     ...
     * birthDate and the second as the asOf.  DeathDate for asOf gives age at time of death.
     * Today for asOf gives age today.
     *
    ...
    public static int[] age( BigDate birthDate, BigDate asOf )
        ...
        int birthYYYY = birthDate.yyyy;
        int birthMM = birthDate.mm;
        int birthDD = birthDate.dd;
        int asOfYYYY = asOf.yyyy;
        int asOfMM = asOf.mm;
        int asOfDD = asOf.dd;
     ...
     *
     * @param yyyy1 first year
     * @param mm1   first month 1..12
     * @param dd1   first day 1..31
     * @param yyyy2 second year
     * @param mm2   second month 1..12
     * @param dd2   second day 1..31
     *
     * @return +ve if date1 > date2, -ve if date1 < date2 and 0 if date1 == date2
     * @see #compareTo(Object)
     */
    public static int compare( int yyyy1, int mm1, int dd1, int yyyy2, int mm2, int dd2 )
        {
    ...
    public static int dayOfWeek( int ordinal )
        ...
        return ( ordinal == NULL_ORDINAL )
               ? 0
               : ( ( ordinal + SUNDAY_IS_ZERO_ADJUSTMENT - MIN_ORDINAL ) % 7 );
        } // /method
    ...
    public static int daysInMonth( int mm, boolean leap )
        ...
        if ( mm != 2 )
            {
            return USUAL_DAYS_PER_MONTH_TABLE[ mm - 1 ];
            }
        else
            {
            return leap ? 29 : 28;
     ...
     * How many days are there in a given month?
     *
     * @param mm   month 1 to 12 (not 0 to 11 as in Sun's Date), no lead 0, or you will get octal.
     * @param yyyy year of interest.
     *
     * @return how many days are in that month
     */
     ...
     *
     * @return (multiplicand times multiplier) / divisor
     */
    ...
    public static String getCopyright()
        {
        return "BigDate 5.1 freeware Copyright: (c) 1997-2017 Roedy Green, Canadian Mind Products, " +
               "http://mindprod.com roedyg@mindprod.com";
        } // /method

    /**
     * Determine if this date is someone's birthdate, or the anniversary of their death, or the anniversary
     * of a marriage etc.
     * For example boolean todayIsYourBirthday =  BigDate.isAnniversary( born, BigDate.localToday() );
     * boolean todayIsYourWeddingAnniversary =  BigDate.isAnniversary( weddingDate, BigDate.localToday() );
     * boolean todayAnniversaryDeathOfBach =  BigDate.isAnniversary( new BigDate("1750-07-28"), BigDate.localToday());
     * <p/>
     * To compute relative anniversary between two dates, use the first as the
     ...
     * Feb 28
     * in non-leap years. If the asOf year is not a leap year then Feb 28 is considered a match
     * for Feb 29. However, if the asOf date is Feb 29 then Feb 28 in the birth year is not considered a match.
     *
     * @param birthDate usually the birth of a person, or possibly a date.
     * @param asOf      usually today, the day you want the to know if it is an anniversary. asof must come after
     *                  birthDate to get a
     *                  meaningful result.
     *
     * @return true if asOf date in as even number of years after the birthdate.  If the asOf date is <= birthDate it
     * will
     * return false.
     * @see #age
     */
    public static boolean isAnniversary( BigDate birthDate, BigDate asOf )
        ...
        return birthDD == 29 && asOfDD == 28 && birthMM == 2 /* && asOfMM == 2 */ && !isLeap( asOf.yyyy );
        } // /method
    ...
    public static boolean isLeap( int yyyy )
        ...
        // jan01OfYear, toGregorian, MONDAY_IS_ZERO_ADJUSTMENT,
        // SUNDAY_IS_ZERO_ADJUSTMENT,
        // AD_EPOCH_ADJUSTMENT and BC_EPOCH_ADJUSTMENT
    ...
    public static int isoDayOfWeek( int ordinal )
        ...
        return ( ordinal == NULL_ORDINAL )
               ? 0
               : ( ( ordinal + MONDAY_IS_ZERO_ADJUSTMENT - MIN_ORDINAL ) % 7 ) + 1;
        } // /method
     ...
     *
     * @return BigDate object initialised to today, local time. You may modify it further.
     * @see #today
     * @see #UTCToday
     * @see #isCelebrating
     */
     ...
     * @throws NumberFormatException id the string has non-numeric chars.
     */
     ...
     * Also handles AD/BC, 1-digit months, 1-digit days and 1, 2 or 3-digit years.  Date must be a valid normalised
     * date.
     *
        ...
        switch ( ST.countInstances( incomplete, '-' ) )
            ...
            case 1:
                ...
                if ( incomplete.endsWith( "BC" ) || incomplete.endsWith( "AD" ) )
                    {
                    complete = incomplete.substring( 0, incomplete.length() - 2 ) + "-01" + incomplete.substring(
                            incomplete.length() - 2 );
                    }
                else
                    {
            ...
            case 0:
                ...
                if ( incomplete.endsWith( "BC" ) || incomplete.endsWith( "AD" ) )
                    {
                    complete = incomplete.substring( 0, incomplete.length() - 2 ) + "-01-01" + incomplete.substring(
                            incomplete.length() - 2 );
                    }
                else
                    {
            ...
            default:
                throw new NumberFormatException(
                        "BigDate.parseYYYYmmdd expects YYYY-MM-DD or YYYY-MM or YYYY." );
            }
        return new BigDate( complete );
        } // /method

    /**
     * Convert ISO date string in form YYYY-MM-DD into days since the 1970-01-01. This method lets you convert
     * directly from
     * Gregorian to ordinal without creating a BigDate object. yyyy-mm-dd must be a valid date.
     *
     * @param yyyy_mm_dd iso date string.
     *
     * @return ordinal, days since 1970-01-01
     */
    public static int toOrdinal( String yyyy_mm_dd )
        {
        final int[] ymd = relaxedParse( yyyy_mm_dd );
        if ( ymd == null )
            {
    ...
    public static int toOrdinal( int yyyy, int mm, int dd )
        ...
        int missingDayAdjust =
                ( yyyy == OJC_LAST_YYYY && ( ( mm == OJC_LAST_MM
                                               && dd > OJC_LAST_DD )
                                             || mm > OJC_LAST_MM ) )
                ? MISSING_DAYS
                : 0;
     ...
     *
     * @return new BigDate object initialised to today at given TimeZone. You may modify it further.
     * @see #localToday
     * @see #UTCToday
     */
     ...
     *
     * @return this the original modified BigDate object.
     */
    public final BigDate addDays( int days )
        {
        if ( days == 0 )
            {
            return this;
     ...
     * standard clone.
     *
     * @return Because this is JDK 1.1 we return Object, not BigDate. You must cast.
     */
    public Object clone()
        {
        try
     ...
     * Compare this BigDate with another BigDate.
     * Compares ordinal.
     ...
     * @see #compare(int, int, int, int, int, int)
     */
    public final int compareTo( /** @NotNull **/ Object other )
        {
        // Integer.compare only available in 1.7
    ...
    public final boolean equals( Object d )
        {
        return d == this || d instanceof BigDate && ( ordinal == ( ( BigDate ) d ).ordinal );
        } // /method
    ...
    public final int getISOWeekNumber()
        {
        if ( ordinal < ORDINAL_LEAP_RULE_100_START )
            {
            return 0;
            }
        int jan04Ordinal = jan01OfYear( yyyy ) + 3;
        int jan04DayOfWeek =
                ( jan04Ordinal + MONDAY_IS_ZERO_ADJUSTMENT - MIN_ORDINAL )
                % 7;// 0=Monday
        // 6=Sunday
    ...
    /**
     * Get milliseconds since 1970-01-01 00:00 GMT for this BigDate. Does not account for leap seconds primarily
    ...
    public final long getTimeStamp( TimeZone timeZone )
         ...
         */
        int offsetInMillis =
                timeZone.getOffset( ordinal >= ORDINAL_0001_01_01AD ? 1 : 0,
                        yyyy,
                        mm - 1,
                        dd,
    ...
    /**
     * If this BigDate represents the date a holiday occurs, usually in this year,
     * it tells you if today is that holiday.  You can allow some slop to include
     * before days before and after days after.  If you want it it to return true
     * only when it is the exact holiday, set them both to 0 or use
     * compareTo ( BigDate.localToday() )
     *
     * @param before how may days prior to include as celebrating.
     * @param after  how many days after ot include as praying.
     *
     * @return true today we are celebrating that holiday .
     * @see #localToday
     */
    public boolean isCelebrating( int before, int after )
        {
        final int today = localToday().getOrdinal();
        return this.ordinal - before <= today && today <= this.ordinal + after;
        } // /method

    /**
     * is this a null BigDate?
     */
    ...
    /**
     * get days since 1970-01-01 for this BigDate. 1970-01-01 = day 0.
     * synomym for getOrdinal.  Parallels enum.ordinal
     *
     * @return days since 1970-01-01. This is NOT what you want for the java.util.Date constructor.
     * @see #getLocalTimeStamp
     * @see #getUTCTimeStamp
    ...
    public final void set( int yyyy, int mm, int dd, int how )
        ...
        this.mm = mm;
        this.dd = dd;
        this.how = how;// save how for future short circuit.
        switch ( how )
            {
            case CHECK:
                if ( !isValid( yyyy, mm, dd ) )
    ...
    public String toString()
        ...
        sb.append( "-" );
        sb.append( ST.toLZ( mm, 2 ) );
        sb.append( "-" );
        sb.append( ST.toLZ( dd, 2 ) );
        if ( yyyy < 1000 )
            {
            if ( yyyy < 0 )
                {
